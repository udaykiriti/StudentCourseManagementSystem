...
import { GraduationCap, Menu, LogOut, User, ChevronRight, ChevronDown, BookOpen, Users, Settings, MessageSquare } from 'lucide-react';
...
const getMenuIcon = (mid) => {
  switch(mid) {
    case 'A001': return <BookOpen className="w-4 h-4" />;
    case 'A101': return <Users className="w-4 h-4" />;
    case 'A102': return <Settings className="w-4 h-4" />;
    case 'A201': return <MessageSquare className="w-4 h-4" />;
...
class AdminHome extends React.Component {
    constructor() {
        ...
        this.state = {
            ...
            menuItems: [], // Always initialize as empty array
            subMenus: {},
            expandedMenus: {},
            selectedModule: '',
            titlebarText: '',
        ...
        console.log('Session ID:', this.sid);
    ...
    loadMenuData = () => {
        const url = "http://localhost:5000/amenu";
        callApi("POST", url, null, this.loadAMenu, errorResponse);
    }
    ...
    loadAMenu = (res) => {
        try {
            ...
            // Keep menuItems as empty array on error
            this.setState({ menuItems: [] });
        }
    }
    ...
    showAMenu = (mid) => {
        ...
        const surl = "http://localhost:5000/amenus"; 
        const ipdata = {
            mid: mid
        };
        
        callApi("POST", surl, ipdata, (res) => this.loadAMenus(res, mid), errorResponse);
    ...
    loadAMenus = (res, mid) => {
        try {
            const data = JSON.parse(res);
            // Ensure data is always an array
            const subMenuItems = Array.isArray(data) ? data : [];
            this.setState(prevState => ({
                subMenus: {
                    ...prevState.subMenus,
                    [mid]: subMenuItems
    ...
    loadAModule = (smid, smtitle) => {
        ...
        switch(smid) {
            case "A00101":
                ...
                break;
            case "A00102":
                moduleSrc = "/admincomponents/deletecourse";
                titleText = "Delete Course";
                break;
            case "A10102":
                moduleSrc = "/admincomponents/viewfaculty";
                titleText = "View Faculty";
                break;  
            case "A10101":
                moduleSrc = "/admincomponents/addfaculty";
                titleText = "Add Faculty";
                break;     
            case "A10103":
                moduleSrc = "/admincomponents/deletefaculty";
                titleText = "Delete Faculty";
                break;
            case "A10201":
            ...
            case "A10202":
                ...
                break;   
            case "A20101":
                moduleSrc = "/admincomponents/viewfeedback";
    ...
    render() {
        ...
        // Additional safety check - ensure menuItems is always an array
                                ...
                                {safeMenuItems.length > 0 ? (
                                    safeMenuItems.map((item) => (
                                        <div key={item.mid} className="border border-gray-200 rounded-xl overflow-hidden">
                                            <button
                                                onClick={() => this.showAMenu(item.mid)}
                                            ...
                                            {expandedMenus[item.mid] && (
                                                <div className="bg-white border-t border-gray-100">
                                                    ...
                                                    ) : subMenus[item.mid] && Array.isArray(subMenus[item.mid]) && subMenus[item.mid].length > 0 ? (
                                                        subMenus[item.mid].map((subItem, index) => (
                                                            <button
                                                                key={subItem.smid}
                                                                onClick={() => this.loadAModule(subItem.smid, subItem.smtitle)}
                            ...
                            {selectedModule ? (
                                ...
                                <div className="flex items-center justify-center h-full text-gray-500">
                                    <div className="text-center">
                                        ...
                                        <p className="text-lg font-medium mb-2">Welcome to Admin Dashboard</p>
